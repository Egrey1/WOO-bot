...
class CollectCommand(Cog):
    ...
    async def collect(self, ctx: Context):
        ...
        if random.randint(0, 100) <= 1:
            ...
            await ctx.author.send('Так-так... Кажется, Егрей начинает что-то понимать, я повысил шанс удачного вызова команды !version, а я напоминаю, там находятся первые два символа пароля. Последние два это 04. Центральный символ мне неизвестен и тебе придется самому его отгадать`.` Эта игра слишком затянулась`.` `.` `.` ')
